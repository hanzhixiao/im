...
func (r *userRepository) Create(user *po.User) (err error) {
	...
	if user.ImId == "" {
		user.ImId = xsnowflake.DefaultimId()
	}
	db := xmysql.GetDB()
...
func (r *userRepository) UserList(w *entity.MysqlQuery) (list []*pb_user.UserInfo, err error) {
	...
	db := xmysql.GetDB()
	err = db.Where(w.Query, w.Args...).Select("uid,im_id,status,nickname,firstname,lastname,gender,birth_ts,mobile,city_id").Find(&list).Error
	return
}
...
func (r *userRepository) TxBasicUserList(tx *gorm.DB, w *entity.MysqlQuery) (list []*pb_user.BasicUserInfo, err error) {
	...
	err = tx.Model(po.User{}).Select("uid,im_id,nickname,gender,birth_ts,city_id,avatar").Where(w.Query, w.Args...).Find(&list).Error
	return
}
...
func (r *userRepository) BasicUserInfo(w *entity.MysqlQuery) (user *pb_user.BasicUserInfo, err error) {
	...
	db := xmysql.GetDB()
	err = db.Model(po.User{}).Select("uid,im_id,nickname,gender,birth_ts,city_id,avatar").Where(w.Query, w.Args...).Find(&user).Error
	return
}
...
func (r *userRepository) BasicUserInfoList(w *entity.MysqlQuery) (list []*pb_user.BasicUserInfo, err error) {
	list = make([]*pb_user.BasicUserInfo, 0)
	db := xmysql.GetDB()
	err = db.Model(po.User{}).Select("uid,im_id,nickname,gender,birth_ts,city_id,avatar").Where(w.Query, w.Args...).Find(&list).Error
	return
}
