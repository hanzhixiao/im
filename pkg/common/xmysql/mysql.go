...
func spliceDsnWithDefaultDatabase(c *conf.Db) (dsn string) {
	dsn = fmt.Sprintf("%s:%s@(%s)/%s?charset=utf8&parseTime=true&loc=Local",
		c.Username,
		c.Password,
		c.Address, "mysql")
...
func ConnectDB(cfg *conf.Mysql) (db *gorm.DB, err error) {
	...
	for i, c := range cfg.Sources {
		dsn = spliceDsnWithDefaultDatabase(c)
		db, err = gorm.Open(mysql.Open(dsn), opts)
		if err != nil {
			xlog.Error(err.Error())
			return
		}
		sql := fmt.Sprintf("CREATE DATABASE IF NOT EXISTS %s default charset utf8mb4 COLLATE utf8mb4_unicode_ci;", c.Db)
